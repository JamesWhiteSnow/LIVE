...
BlockFile::BlockFile(char *name, int b_length)
{
	...
	else
	{
		if (blocklength < BFHEAD_LENGTH)
			error(const_cast<char *>("BlockFile::BlockFile: Blocks zu kurz\n"), TRUE);

		fp = fopen(filename, "wb+");
		if (fp == NULL)
			error(const_cast<char *>("BlockFile::new_file: Schreibfehler"), TRUE);
	...
	: BlockFile(name, blength)
{
	...
	if (csize >= 0)
		...
		error(const_cast<char *>("Cache size cannot be negative"), TRUE);
...
void CachedBlockFile::set_cachesize(int size)
{
	...
	if (size >= 0)
	{
		...
		error(const_cast<char *>("Cache size cannot be negative\n"), TRUE);
}
