...
double *overlapRect(int dimension, double *r1, double *r2)
{
    ...
    for (int i = 0; i < dimension; i++)
    {
        ...
        overlap[2 * i] = std::max(r1[i * 2], r2[i * 2]);
        overlap[2 * i + 1] = std::min(r1[i * 2 + 1], r2[i * 2 + 1]);
    }
...

{
    ...
    *mbr = new double[2 * dimension];
    ...
    {
        (*mbr)[i] = std::min(r1[i], r2[i]);

        (*mbr)[i + 1] = std::max(r1[i + 1], r2[i + 1]);
    }
}
        ...
        if (bl[i].minmaxdist < bl[aktlast - 1].mindist)
            ...
            for (j = 0; (j < aktlast); j++)
                if ((i != j) && (bl[j].mindist > bl[i].minmaxdist))
                {
                    ...
                    if (j <= i)
                        error(const_cast<char *>("Error in pruneBrunchList\n"), true);
                    break;
                }
    }
