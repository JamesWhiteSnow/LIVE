...

{
	...
	{
	    if (!akt)
		error(const_cast <char*>("LinList::get: List seems to be inkonsistent"), TRUE);
	    akt = akt->next;
	}
    }
    else
...
void LinList::check()
{
    ...
    {
	if (last != NULL)
	    error(const_cast <char*>("LinList::check: first == NULL, last != NULL"), FALSE);
	if (anz != 0)
	    error(const_cast <char*>("LinList::check: first == NULL, anz != 0"), FALSE);
	return;
    }
    ...
    if (old_f->prev != NULL)
    {
	error(const_cast <char*>("LinList::check: Listenkopf.prev ungleich NULL"), FALSE);
	return;
    }
	...
	if (f->prev != old_f)
	{
	    error(const_cast <char*>("LinList::check: Rueckwaertsverkettung fehlerhaft"), FALSE);
            return;
        }
	if (old_f->next != f)
	{
	    error(const_cast <char*>("LinList::check: Vorwaertsverkettung fehlerhaft"), FALSE);
            return;
        }
	old_f = f;
    ...
    if (old_f->next != NULL)
    {
	error(const_cast < char*>("LinList::check: Listenende.next ungleich NULL"), FALSE);
	return;
    }

    if (last != old_f)
    {
	error(const_cast < char*>("LinList::check: last ungleich Listenende"), FALSE);
	return;
    }
